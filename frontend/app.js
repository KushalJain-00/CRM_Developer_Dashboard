          ...
          callLogsList.innerHTML = data.logs.map(log => {
            ...
            return `<div class="call-log-entry">
              <div class="cle-header">
                <span class="cle-type badge ${typeCls}">${escapeHTML(log.call_type || '')}</span>
                <span class="cle-outcome">${escapeHTML(log.outcome || '')}</span>
                <span class="cle-date">${dateStr}</span>
              </div>
              <div class="cle-notes">${escapeHTML(log.notes || '')}</div>
            </div>`;
          }).join('');
        } else {
